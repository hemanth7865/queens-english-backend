...
export class BatchController {
    ...
    async reBatch(request: Request, response: Response, next: NextFunction) {
        if (!request.body.studentId) {
            return {
                status: 400,
                errors: ["Please Provide Correct Student Information"],
            };
        }
        var studentAlreadyExists;
        if (request.body.batchId) {
            studentAlreadyExists = await this.batchService.checkStudentExistsInBatch(
                request.body
            );
            ...
            if (studentAlreadyExists.data.length > 0) {
                return {
                    status: 400,
                    errors: [
                        `Student ${studentAlreadyExists.data.firstName} ${studentAlreadyExists.data.middleName} ${studentAlreadyExists.data.lastName} of class ${studentAlreadyExists.data.classSection} already exist in the selected batch.`,
                    ],
                };
            }
        }
        ...
        try {
            ...
            batch = {
                status: 400,
                errors: ["Something went wrong while creating/updating the batch."],
            };
        }
        return batch;
    }

    async bulkReBatchStudents(
        request: Request,
        response: Response,
        next: NextFunction
    ) {
            ...
            const result = await this.reBatch(
                {
                    body: {
                        ...
                        bulkRebatch: request.body.bulkRebatch
                            ? request.body.bulkRebatch
                            : false,
    ...
    async getBatchDetails(
        ...
        try {
            await this.batchService
                .syncStudentsFromCosmosToSQLByBatchId(request.params.id)
    ...
    async remove(request: Request, response: Response, next: NextFunction) {
        try {
            let classesToRemove = await this.classesRepository.findOne(
                request.params.id
            );
    ...
    async updateBatchZoomInfoAndWACSV(
        request: Request,
        response: Response,
        next: NextFunction
    ) {
    ...
    async checkStudentBatches(
        ...
        let isPresent: any;
        try {
            isPresent = await this.batchService.checkStudentBatches(
                data.students,
                data.id
            );
    ...
    async bulkRemoveStudentsFromBatch(
        request: Request,
        response: Response,
        next: NextFunction
    ) {
        try {
            return await this.batchService.removeStudents(
    ...
    async changeActiveLesson(request: Request, response: Response) {
        try {
            ...
            const res = await this.batchService.changeActiveLesson(
                batchId,
                lessonNumber,
                reason,
                user
    ...
    async updateAllBatchesWithNoSchoolIdButTeacherHas(
        ...
        try {
            const res =
                await this.batchService.updateAllBatchesWithNoSchoolIdButTeacherHas();
    ...
    async syncStudentsFromCosmosToSQL(
        request: Request,
        response: Response,
        next: NextFunction
    ) {
        try {
            this.batchService.syncAllStudentsFromCosmosToSQL(),
                response
                    .status(200)
                    .send({ message: "Started syncing students from cosmos to SQL" });
        } catch (error) {
            response.status(500).send({
                message:
                    error?.message || "Some error occurred while updating the batch.",
