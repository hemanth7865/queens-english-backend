    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        if (!(await queryRunner.hasTable("school"))) {
            await queryRunner.query(
                "CREATE TABLE `school` (`id` VARCHAR(255) NOT NULL,`schoolName` VARCHAR(255) NOT NULL,`schoolCode` VARCHAR(255) NOT NULL, `poc` VARCHAR(255),`sraId` VARCHAR(255) NOT NULL, `createdAt` DATE,`schoolStatus` ENUM('Active', 'Inactive') NOT NULL DEFAULT 'Active',PRIMARY KEY(`id`));"
            );
        }
    }
